...
interface IEscrow {
    ...
    struct Intent {
        ...
        bytes32 fiatCurrency; // Currency code that the owner is paying in offchain (keccak256 hash of the currency
            // code)
    ...
    event IntentReleased(
        uint256 indexed intentId, uint256 indexed depositId, address owner, address to, uint256 amount
    );

    event IntentCancelled(uint256 intentId);

    event RedeemRequestSignaled(uint256 indexed redeemId, address indexed owner, uint256 amount, string accountNumber);

    event RedeemRequestFulfilled(uint256 redeemId);

    event RedeemRequestCancelled(uint256 redeemId);

    event DepositCreated(uint256 depositId, address depositor, IERC20 token, uint256 amount, Range intentAmountRange);

    event DepositVerifierAdded(uint256 indexed depositId, address indexed verifier, bytes32 payeeDetailsHash);

    event DepositCurrencyAdded(
        uint256 indexed depositId, address indexed verifier, bytes32 indexed currency, uint256 conversionRate
    );

    event DepositConversionRateUpdated(
        uint256 indexed depositId, address indexed verifier, bytes32 fiatCurrency, uint256 newConversionRate
    );

    event DepositIncreased(
        uint256 indexed depositId, address indexed depositor, uint256 additionalAmount, uint256 newTotalAmount
    );
    ...
    event DepositIntentAmountRangeUpdated(uint256 indexed depositId, Range oldRange, Range newRange);
