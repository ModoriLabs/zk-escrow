...
abstract contract BaseEscrowUpgradeableTest is BaseTest {
    ...
    function setUp() public virtual override {
        ...
        bytes memory initData =
            abi.encodeWithSelector(EscrowUpgradeable.initialize.selector, owner, INTENT_EXPIRATION_PERIOD, CHAIN_NAME);
    ...
    // Helper function to get deposit struct data
    function _getDeposit(uint256 depositId)
        ...
        returns (
            ...
            IERC20 token,
            uint256 amount,
            IEscrow.Range memory intentAmountRange,
            bool acceptingIntents,
            uint256 remainingDeposits,
            uint256 outstandingIntentAmount
        )
    {
        return escrow.deposits(depositId);
    }

    // Helper function to get intent struct data
    function _getIntent(uint256 intentId)
        internal
        view
        returns (
            address owner,
    ...
    {
        return escrow.intents(intentId);
    }

    // Helper function to signal an intent
    function _signalIntent(
        address user,
        uint256 depositId,
        uint256 amount,
        address recipient
    ...
    {
        vm.prank(user);
        escrow.signalIntent(depositId, amount, recipient, address(tossBankReclaimVerifierV2), keccak256("KRW"));
        intentId = escrow.accountIntent(user);
    }
}
